...
class ModelTrainer:
    def __init__(self,config_path,data_path):
        try:
            ...
            self.experiment = comet_ml.Experiment(
                api_key="CO6ROcKkZKBuBK8DhbW8APhJg",
                project_name="mlops-2",
                workspace="johnstevel"
            )
            logger.info("Model Training & COMET_ML Initialized")
        except Exception as e:
            raise CustomException("Error in Loading Configuration",e)
    
    def RecommenderNet(self,n_users,n_anime):
        try:

            embedding_size = self.config["model_trainer"]["embedding_size"]
    ...
    def train_model(self):
        try:
            ...
            model = self.RecommenderNet(n_users=n_users,n_anime=n_anime)
